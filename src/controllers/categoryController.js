...
export const categoryCreate = async(req, res, next) => {
   try {
    ...
    if(category){
      throw new CustomError(`category with name ${category.category_name} already exists`, 404)
    }
...
export const updateCategory = async(req, res, next) => {
  try {
    ...
    const category = await Category.findByIdAndUpdate(id, {category_name, description},{new:true})
    if(!category){
